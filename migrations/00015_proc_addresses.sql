...
CREATE FUNCTION get_countries()
RETURNS TABLE(code TEXT, name TEXT) AS $$
  SELECT country_code, country_name
  FROM Countries;
$$ LANGUAGE sql SECURITY DEFINER;

CREATE FUNCTION get_counties(p_country_name TEXT)
...
CREATE FUNCTION get_cities(p_country_name TEXT, p_county_name TEXT)
RETURNS TABLE(name TEXT) as $$
    SELECT city_name
    FROM Cities City
...
CREATE FUNCTION create_user_address(
  p_token TEXT,
...
CREATE FUNCTION get_user_addresses(p_token TEXT)
RETURNS result_t AS $$
DECLARE
    user_session JSON;
    address_data JSONB;
...
-- +goose Down
-- +goose StatementBegin
DROP FUNCTION get_user_addresses;
DROP FUNCTION create_user_address;
DROP FUNCTION get_cities;
DROP FUNCTION get_counties;
DROP FUNCTION get_countries;
-- +goose StatementEnd
