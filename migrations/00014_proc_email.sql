-- +goose Up
-- +goose StatementBegin
CREATE FUNCTION is_valid_email(p_email TEXT)
RETURNS BOOLEAN AS $$
BEGIN
  IF LENGTH(p_email) - LENGTH(REPLACE(p_email, '@', '')) != 1 THEN
    RETURN FALSE;
  END IF;

  IF p_email !~ '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$' THEN
    RETURN FALSE;
  END IF;

  RETURN TRUE;
END;
$$ LANGUAGE plpgsql IMMUTABLE STRICT;

CREATE FUNCTION normalize_email(p_email TEXT)
RETURNS TEXT AS $$
DECLARE
  local_part TEXT;
  domain_part TEXT;
  plus_pos INT;
BEGIN
  IF NOT is_valid_email(p_email) THEN
    RETURN '';
  END IF;

  local_part := LOWER(SPLIT_PART(p_email, '@', 1));
  domain_part := LOWER(SPLIT_PART(p_email, '@', 2));

  plus_pos := POSITION('+' IN local_part);
  IF plus_pos > 0 THEN
    local_part := left(local_part, plus_pos - 1);
  END IF;

  RETURN local_part || '@' || domain_part;
END;
$$ LANGUAGE plpgsql IMMUTABLE STRICT;

CREATE FUNCTION exists_email(p_email TEXT)
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    SELECT 1 FROM Users WHERE email = normalize_email(p_email)
  );
$$ LANGUAGE sql SECURITY DEFINER;
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP FUNCTION exists_email;
DROP FUNCTION normalize_email;
DROP FUNCTION is_valid_email;
-- +goose StatementEnd
